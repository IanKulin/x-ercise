...
function validateImportData(data: any): { valid: boolean; error?: string } {
  ...
  for (let i = 0; i < data.exercises.length; i++) {
    ...
    if (!ex.name || typeof ex.name !== "string") {
      return {
        valid: false,
        error: `Exercise ${i}: missing or invalid "name"`,
      };
    }
    ...
    if (
      ...
      return {
        valid: false,
        error: `Exercise ${i}: duration must be 1-3600 seconds`,
      };
    }
    ...
    if (!ex.description || typeof ex.description !== "string") {
      return {
        valid: false,
        error: `Exercise ${i}: missing or invalid "description"`,
      };
    }
    ...
    if (ex.imageSlug && typeof ex.imageSlug === "string") {
      if (!isValidImageSlug(ex.imageSlug)) {
        return {
          valid: false,
          error: `Exercise ${i}: invalid imageSlug format`,
        };
      }
    }
    ...
    if (ex.position !== undefined && typeof ex.position !== "number") {
      return {
        valid: false,
        error: `Exercise ${i}: position must be a number`,
      };
    }
  }
...
export default (logger: Logger): Router => {
  ...
  router.get("/", (req: Request, res: Response) => {
    try {
      ...
      const sets = stmt.all() as (ExerciseSetRow & {
  ...
  router.put("/sets/:id", (req: Request, res: Response) => {
    try {
      ...
      const existingExercisesStmt = db.prepare(
        "SELECT image_slug FROM exercises WHERE set_id = ?",
      );
      const existingExercises = existingExercisesStmt.all(
        setId,
      ) as ExerciseRow[];

      // Delete all existing exercises (will re-insert)
      const deleteExercisesStmt = db.prepare(
        "DELETE FROM exercises WHERE set_id = ?",
      );
      deleteExercisesStmt.run(setId);

      // Clean up images that are no longer used
      const newImageSlugs = new Set(
        exercises.map((e) => e.imageSlug).filter(Boolean),
      );
  ...
  router.get("/sets/:id/export", (req: Request, res: Response) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${set.slug}.json"`,
      );
    ...
    (req: Request, res: Response) => {
      try {
        ...
        const checkSlugStmt = db.prepare(
          "SELECT id FROM exercise_sets WHERE slug = ?",
        );
