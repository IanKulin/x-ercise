...
describe('API Tests', () => {
    ...
    describe('GET /', () => {
        ...
        it('should show completion count for user with history', async () => {
            // Insert test data
            db.prepare('INSERT INTO completions (set_slug, username, completed_at) VALUES (?, ?, ?)')
        ...
        it('should show correct data when user completed some sets but not others', async () => {
            ...
            const response = await fetch('http://localhost:3000/?username=partialuser');
            const body = await response.text();
            ...
            // Cleanup
            db.prepare('DELETE FROM completions WHERE username = ?').run('partialuser');
        });
    });
